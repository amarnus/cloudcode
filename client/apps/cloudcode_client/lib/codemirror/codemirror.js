...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
      ...
      for (var n = e.target(); n != div; n = n.parentNode)
        if (n.parentNode == gutterText) {
          if (options.onGutterClick) 
          	// Pass the onMouseDown event to the onGutterClick callback
    ...
    function pageCoords(pos) {
      var local = localCoords(pos, true), off = eltOffset(lineWrap);
    ...
    function posFromMouse(e, liberal) {
      var off = eltOffset(lineWrap),
          x = e.pageX() - off.left,
          y = e.pageY() - off.top;
      if (!liberal && !(e.target() == div && y > (lines.length * lineHeight())))
    ...
    var instance = {
      ...
      localCoords: function(pos) { return localCoords(clipPos(pos), false); },
      scrollIntoView: function(x1,y1,x2,y2) { scrollIntoView(x1,y1,x2,y2); },
      ...
      cursorCoords: function(start){return pageCoords(start ? sel.from : sel.to);},
  ...
  CodeMirror.getMode = function(options, spec) {
    if (typeof spec == "string" && /^[^\s\/]+\/[^\s\/]+$/.test(spec)) {
